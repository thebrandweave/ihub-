...
switch ($action) {
    ...
    default:
        try {
            // --- 1. CLEANUP LOGIC: Delete records older than 30 days ---
            $cleanupStmt = $pdo->prepare("DELETE FROM notifications WHERE user_id = ? AND created_at < DATE_SUB(NOW(), INTERVAL 30 DAY)");
            ...
            echo json_encode(['success' => false, 'error' => 'Database error', 'items' => []]);
        }
        exit;
}