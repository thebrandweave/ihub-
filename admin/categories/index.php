...
$error = $_GET['error'] ?? null; // LINE 7 IS CLEANED
...
// Function to handle image path retrieval (assuming BASE_URL is set in config.php)
...
// --- Calculate statistics for dashboard (Simplified without status) ---
$totalCategoriesCount = $pdo->query("SELECT COUNT(*) FROM categories")->fetchColumn() ?? 0;

// The count of products *without* a category ID is needed for dashboard insight.
    ...
    <div class="category-card group">
        <div class="bg-white rounded-xl shadow-md border border-gray-200 relative h-64 overflow-hidden transition-all duration-300 hover:shadow-xl hover:border-red-300">
            ...
            <img src="<?= getImagePath($c['image_url']) ?>"
                ...
                onerror="this.onerror=null;this.src='https://via.placeholder.com/400x300?text=Category'">
            ...
            <div class="absolute inset-0 flex flex-col justify-between p-5">
                ...
                <div>
                    ...
                    <p class="text-sm text-white/90 flex items-center gap-1 drop-shadow-md">
                        ...
                        <?= number_format($c['total_products'] ?? 0) ?> Products
                    </p>
                </div>
